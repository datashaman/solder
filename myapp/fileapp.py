...
class FileApp(DataApp):
    ...
    def __init__(self, filename, headers=None, **kwargs):
        ...
        self.content_type, self.content_encoding = self.guess_type()
        if self.content_type and 'content_type' not in kwargs:
            kwargs['content_type'] = self.content_type
        if self.content_encoding and 'content_encoding' not in kwargs:
            kwargs['content_encoding'] = self.content_encoding
    ...
    def __call__(self, environ):
        ...
        if app is None:
            ...
            if not os.path.normpath(path).startswith(self.path):
                ...
                self.content_type = app.content_type
                self.content_encoding = app.content_encoding
            else:
